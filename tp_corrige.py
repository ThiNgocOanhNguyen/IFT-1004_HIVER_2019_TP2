...
def creer_dict_mot(mot, lettres):
    """
    Cette fonction va obtenir la représentation en dictionnaire
    d'un mot à partir de sa liste de lettres uniques. Le
    dictionnaire aura comme clés les différentes lettres uniques
    ...
    joueur. Un exemple de dictionnaire est présenté plus bas.

    Pour obtenir la représentation en dictionnaire, une façon
    simple de procéder est de commencer par initialiser un
    dictionnaire vide. On parcourt ensuite la liste des lettres
    uniques une lettre à la fois. Pour la lettre actuellement
    utilisée dans la boucle, on initialise l'entrée du
    dictionnaire avec la lettre courante comme clé. On parcourt
    ensuite le mot une lettre à la fois et on ajoute les index où
    la lettre du mot est la même que la lettre courante à
    l'entrée correspondante dans le dictionnaire du mot.
    ...
    Returns: 
        Une représentation en dictionnaire du mot en entrée.
        Pour le dictionnaire, les clés sont des caractères
        ...
        le dictionnaire retourné serait
...
def jouer_manche(dict_mot_choisi, taille_mot):
    ...
    Args:
        dict_mot_choisi : la représentation en dictionnaire du mot
            choisi
...
def afficher_etat_manche(dict_mot, n_prises, max_prises, taille_mot):
    ...
    Args:
        dict_mot : La représentation en dictionnaire du mot à
            deviner
...
def manche_terminee(dict_mot, n_prises, max_prises, taille_mot):
    ...
    terminée. Une manche peut être terminée pour deux raisons :
        ...
        1) Toutes les lettres ont été découvertes par le joueur. 
            ...
            dans la représentation en dictionnaire du mot, le booléen
            représentant la découverte de la lettre par l'usager est
            à True, pour toutes les lettres clé du dictionnaire.
    ...
    Args:
        dict_mot : La représentation en dictionnaire du mot
            recherché
...
def demander_lettre_joueur(lettres_tentees):
    ...
    ***Notez ici que vous n'avez pas à ajouter la lettre tentée
    ...
    while True:
        ...
        lettre_tentee = input("Quelle lettre voulez-vous essayer ? ")

        if lettre_tentee in lettres_tentees:
            print("La lettre {} a déjà été tentée !".format(lettre_tentee))
        else:
...
def mise_a_jour_manche(lettre_tentee, dict_mot_choisi, n_prises):
    ...
    Deux cas peuvent survenir :
        ...
        1) Si la lettre tentée est une des clés du dictionnaire du 
...
def test_creer_dict_mot():
    """
    Pour tester l'égalité entre deux dictionnaires, on valide que
    les dictionnaires ont le même nombre de clés, et que les clés
    ...
    assert len(dict_mot_test_attendu.keys()) == len(dict_mot_test.keys()), \
            "Le dictionnaire retourné et le dictionnaire attendu n'ont pas le même nombre de clés !"
            
    for cle_attendue, valeur_attendue in dict_mot_test_attendu.items():
        assert cle_attendue in dict_mot_test, \
            "Une lettre clé attendue n'était pas présente dans le dictionnaire retourné !"
